...
export async function GET(
  ...
  try {
    ...
    // 非 manager 仅可查看自己的 Agent 绑定
    if (currentRole !== 'manager' && data.agent_id !== actor) {
      return NextResponse.json({ error: '无权查看该 Agent' }, { status: 403 });
    }
...
export async function PUT(
  ...
  try {
    ...
    // Webhook URL 更新：仅 Manager 或 Agent 本人可更新
    if (body.webhookUrl !== undefined) {
      if (currentRole !== 'manager' && existing.agent_id !== actor) {
        return NextResponse.json({ error: '无权更新该 Agent 的 Webhook URL' }, { status: 403 });
      }
