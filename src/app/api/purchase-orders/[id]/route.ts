...
import { getUserIdentityWithLookup, filterPurchaseOrders, getRequesterAccessiblePOIds, canCreatePO, type Role } from '@/lib/role-filter';
...
export async function GET(
  ...
  try {
    ...
    if (poError || !po) {
      // 如果用户端查询失败，尝试服务角色（sent 状态的订单可能属于其他用户）
      const { data: poByService, error: poByServiceError } = await serviceClient
        .from('purchase_orders')
        .select('*')
        .eq('id', poId)
        .single();
      
      if (poByServiceError || !poByService) {
        return NextResponse.json({ error: '采购订单不存在' }, { status: 404 });
      }
      
      // 如果是 sent 状态的订单，允许访问
      if (poByService.status !== 'sent') {
